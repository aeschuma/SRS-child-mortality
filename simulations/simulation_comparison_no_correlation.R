...
## This code:
## - simulates data with fixed effects and non-correlated random effects
...
for (exp_ind in 3:length(exposures)) {
    
    if (testing) exp_ind <- 2
    
    ## update status
    cat(paste0("\n Exposure: ",exposures[exp_ind],"\n")); flush.console();
    
    # fixing sims that crash
    sim_start <- 1
    n_alter <- 0
    ...
    for (sim in sim_start:(nsim+n_alter)) {
        ...
        # fixed effects (intercept overall and cause modifier)
        beta <- beta_intercepts
        if (linear_time) beta <- c(beta_intercepts,beta_time)
        dat$alpha <- X%*%beta
        
        # iid overdispersion term
        dat$gamma <- rnorm(nrow(dat),0,gamma_sd)
        ...
        dat$logmx <- dat$alpha
        if (iid_overdisp) dat$logmx <- dat$logmx + dat$gamma
        ...
        if (!casm_only) {
            ...
            if (iid_overdisp) allcause_fmla_mod <- paste(allcause_fmla_mod,"+ f(factor(obs), model='iid')")
            ...
            pred_logu5mr <- log(allmod$summary.fitted.values$`0.5quant`/exp(simdeathsallcause$logpy))
            ...
            if (multinomial) {
                ...
                csmf_fmla_mod <- paste0(csmf_fmla_mod," + f(cause, model='iid', ",
                                        "constr = TRUE, hyper = ",
